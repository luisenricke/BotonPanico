...
@Entity(
    tableName = AlertContact.SCHEMA.TABLE,
    foreignKeys = [
        ForeignKey(
            entity = Alert::class,
            parentColumns = arrayOf(Alert.SCHEMA.ID),
            childColumns = arrayOf(AlertContact.SCHEMA.ALERT_ID),
        ...
        ForeignKey(
            ...
            childColumns = arrayOf(AlertContact.SCHEMA.CONTACT_ID),
            onUpdate = ForeignKey.CASCADE,
            onDelete = ForeignKey.CASCADE
        )
    ]
...
data class AlertContact(
    @ColumnInfo(name = ALERT_ID, index = true)
    val alertId: Long,
